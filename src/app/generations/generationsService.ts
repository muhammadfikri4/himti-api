...
import {
  ...
  getGenerationCount,
  getGenerations,
  getMatchGenerationExceptSameId,
  updateGeneration,
} from "./generationsRepository";
...
export const createGenerationService = async ({
  ...
  if (!year) {
    return new ErrorApp(
      MESSAGES.ERROR.REQUIRED.ANGKATAN_YEAR,
      400,
  ...
  if (matchAngkatan) {
    return new ErrorApp(
      MESSAGES.ERROR.ALREADY.ANGKATAN,
      400,
      MESSAGE_CODE.BAD_REQUEST
    );
  }
  ...
  const response = await createGeneration({
    isActive,
    year: year_string as string,
  });
  return response;
};

export const getGenerationService = async ({
  search,
  page = 1,
  perPage = 10,
  status,
...
export const deleteGenerationService = async ({ id }: GenerationBodyDTO) => {
  ...
  if (isUsed) {
    return new ErrorApp(
      MESSAGES.ERROR.RELATION.ANGKATAN,
      400,
      MESSAGE_CODE.BAD_REQUEST
    );
  }
...
export const updateGenerationService = async ({
  id,
  year,
  isActive,
}: GenerationBodyDTO) => {
  ...
  if (year) {
    const isAlreadyUse = await getMatchGenerationExceptSameId(
      year_string as string
    );
