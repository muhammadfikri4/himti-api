import { Attendance, Point, User } from "@prisma/client";
...
export const historyAbsensiMapper = (
  absensi: EventMeetingData[]
): MeetingHistoryResponse[] => {
  const data: EventMeetingData[] = [];
  absensi.map((item) => {
    if (
      data.find(
        (i: EventMeetingData) => i.eventMeetingId === item.eventMeetingId
      )
    ) {
      return null;
    }
  ...
  const attend = data.map((item) => {
    const meeting = absensi.filter(
      (i) => i.eventMeetingId === item.eventMeetingId
    );
    return {
      eventMeeting: {
      ...
      attendance: meeting.map((meet) => {
        return {
          id: meet.id,
          image: meet.image.includes("https")
            ? meet.image
            : ImagePath(`absensi/${meet.image}`),
...
export const getAttendanceDTOMapper = (data: EventMeetingData) => {
  return {
    ...
    image: data.image.includes("https")
      ? data.image
      : ImagePath(`attendance/${data.image}`),
    meeting: {
      id: data.Meeting.id,
...
export const getAttendanceByMeetingMapper = (data: AttendanceData[]) => {
  return data.map((item) => ({
    id: item.id,
    user: {
      id: item.User.id,
      name: item.User.name as string,
      email: item.User.nim as string,
    },
    location: item.address,
    image: item.image.includes("https")
      ? item.image
      : ImagePath(`attendance/${item.image}`),
    point: item.Point.reduce((a: number, b: Point) => a + b.point, 0),
